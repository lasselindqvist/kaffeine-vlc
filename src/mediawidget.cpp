 ...
 *  video format is changeable using CTRL+A
 *  video scale is changeable using SHIFT+O
 */
...
{
//    Log("MediaWidget::play:url=")<<url.prettyUrl()<<
  //                                 "subtitleUrl="<<subtitleUrl.prettyUrl();
	// FIXME mem-leak
...
void MediaWidget::chainAspectRatio()
{
    if (currentAspectRatio==AspectRatioWidget)
        currentAspectRatio=AspectRatioAuto;
    else
    {
        int l_ar=static_cast<int>(currentAspectRatio)+1;
        currentAspectRatio=static_cast<AspectRatio>(l_ar);
    }
    backend->setAspectRatio(currentAspectRatio);
}
void MediaWidget::switchScale()
{
    backend->toggleVideoScale();
}
...
void MediaWidget::currentAudioStreamChanged(int currentAudioStream)
{
    if (!blockBackendUpdates) {
        ...
        // prevents from re-selecting the 1st track
        //if (backend->getCurrentAudioStream() != currentAudioStream) {
        if (source->getCurrentAudioStream() != currentAudioStream) {
