 ...
 */
/*!
 * Fabien R <theedge456@free.fr>:
 *  subtitles and audio tracks are enabled
 *  additional video formats are available
 *  max volume is 200
 */
...
{
    currentVideoScale=0;
    const char *arguments[] = { "--no-video-title-show", "--overlay" };
...
{
	// 0 <= volume <= 200
    if (libvlc_audio_set_volume(vlcMediaPlayer, volume*2) != 0) {
...
void VlcMediaWidget::setAspectRatio(MediaWidget::AspectRatio aspectRatio)
{
    ...
    libvlc_video_set_scale(vlcMediaPlayer, currentVideoScale);
    // Crop to fit screen
 //   libvlc_video_set_crop_geometry(vlcMediaPlayer, vlcAspectRatio);

}
void VlcMediaWidget::toggleVideoScale()
{
    currentVideoScale++;
...
void VlcMediaWidget::play(const MediaSource &source)
{
 ...
 //           Log("VlcMediaWidget::playMediaSource::Dvb");
		break;
	}
    ...
    if (source.getType()!=MediaSource::Dvb)
...
void VlcMediaWidget::updatePlaybackStatus()
{
    ...
    libvlc_state_t l_state= libvlc_media_player_get_state(vlcMediaPlayer);
    switch (l_state) {
	case libvlc_NothingSpecial:
//            Log("VlcMediaWidget::updatePlaybackStatus:libvlc_NothingSpecial");
    case libvlc_Stopped:
 //           Log("VlcMediaWidget::updatePlaybackStatus:libvlc_Stopped");
        // FIXME vlc state is not updated synchronously in libvlc_media_player_play
        // seems not the case any more
//		if (libvlc_media_player_get_media(vlcMediaPlayer) == NULL) {
    ...
    //	}
		...
		break;
    case libvlc_Opening:
   //         Log("VlcMediaWidget::updatePlaybackStatus:libvlc_Opening");
            break;
    case libvlc_Buffering:
     //       Log("VlcMediaWidget::updatePlaybackStatus:libvlc_Buffering");
    case libvlc_Playing:
  //          Log("VlcMediaWidget::updatePlaybackStatus:libvlc_Playing");
        playbackStatus = MediaWidget::Playing;
        break;
	case libvlc_Paused:
	...
	case libvlc_Error:
		...
		break;
    default:
  //      Log("VlcMediaWidget::updatePlaybackStatus:libvlc_unhandled=")<<l_state;
        break;
...
void VlcMediaWidget::updateMetadata()
{
	...
	if (media != NULL) {
        ...
        // Needs stream infos
        addPendingUpdates(AudioStreams|Subtitles);
	}
}
...
{
	audioStreams.clear();
//    int l_trackNb=libvlc_audio_get_track_count(vlcMediaPlayer);
 //   Log("VlcMediaWidget::updateAudioStreams:l_trackNb=")<<l_trackNb;
 ...
 //   else Log("VlcMediaWidget::updateAudioStreams:track == NULL");
  ...
  //      Log("VlcMediaWidget::updateAudioStreams:audioStream=")<<audioStream;
...
void VlcMediaWidget::vlcEvent(const libvlc_event_t *event)
{
 ...
 //           Log("VlcMediaWidget::vlcEvent:libvlc_MediaMetaChanged");
		pendingUpdatesToBeAdded = Metadata;
		break;
    case libvlc_MediaPlayerOpening:
   //         Log("VlcMediaWidget::vlcEvent:libvlc_MediaPlayerOpening");
            pendingUpdatesToBeAdded = PlaybackStatus;
            break;
    case libvlc_MediaPlayerEncounteredError:
     //       Log("VlcMediaWidget::vlcEvent:libvlc_MediaPlayerEncounteredError");
        pendingUpdatesToBeAdded = PlaybackStatus;
		break;
	...
	case libvlc_MediaPlayerLengthChanged:
       //     Log("VlcMediaWidget::vlcEvent:libvlc_MediaPlayerLengthChanged");
        pendingUpdatesToBeAdded = CurrentTotalTime;
		break;
	case libvlc_MediaPlayerSeekableChanged:
         //   Log("VlcMediaWidget::vlcEvent:libvlc_MediaPlayerSeekableChanged");
        pendingUpdatesToBeAdded = Seekable;
		break;
	case libvlc_MediaPlayerStopped:
           // Log("VlcMediaWidget::vlcEvent:libvlc_MediaPlayerStopped");
        pendingUpdatesToBeAdded = PlaybackStatus;
		break;
	case libvlc_MediaPlayerTimeChanged:
     //       Log("VlcMediaWidget::vlcEvent:libvlc_MediaPlayerTimeChanged");
        pendingUpdatesToBeAdded = CurrentTotalTime;
		break;
	}
